...
int FFGLFBO3D::Create(int _width, int _height, int _depth)
{
  m_width = _width;
  m_height = _height;
  m_depth = _depth;
  ...
  m_glTextureTarget = GL_TEXTURE_3D;
  m_glTextureHandle = 0;
...
int FFGLFBO3D::BindAsRenderTarget()
{
  //make our fbo active
  ...
  if (!IsTextureResident(m_glTextureHandle))
  {
    ...
    glTexParameteri(m_glTextureTarget, GL_TEXTURE_WRAP_R,
        GL_CLAMP_TO_EDGE);
...
int FFGLFBO3D::UnbindAsRenderTarget(GLuint hostFbo )
{
  glBindFramebuffer(GL_FRAMEBUFFER, hostFbo);
  return 1;
}

FFGLTexture3DStruct FFGLFBO3D::GetTextureInfo()
{
  FFGLTexture3DStruct t;
